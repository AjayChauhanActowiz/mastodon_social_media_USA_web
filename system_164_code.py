...
def search_word(search_text):
    ...
    hashtags_list = []
    h_t = search_response.get('hashtags', [])
    if len(h_t)>10:
        l = 10
    if len(h_t)<=10 and len(h_t)!=0:
        l = len(h_t)
    if len(h_t)==0:
        return {
...
total_data = []
for s_t in ['war','morning','jew']:
    total_data.append(search_word(s_t))
    print(f'completed {s_t}')
with open('mastodon_sample_data_search_word_hashtags.json', 'w', encoding='utf-8') as f:
    json.dump(total_data, f, ensure_ascii=False, indent=2)

# total_data = []
# for acc in accounts.accts[:5]:
#     detail = main(acc)
#     print(detail)
...
#             print(f'completed {detail['profile']['acct']}')
#             total_data.append(detail)
#         except Exception as e:
#             print(f"Error processing account {acc}: {e}")
#
# with open('mastodon_sample_data_for_5_username.json', 'w', encoding='utf-8') as f:
#     json.dump(total_data, f, ensure_ascii=False, indent=2)
